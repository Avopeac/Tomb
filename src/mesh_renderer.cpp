	...
	graphics_base_(graphics_base)
{
	...
	vertex_program_ = program_cache.GetFromFile("deferred.vert", GL_VERTEX_SHADER, "assets/shaders/deferred.vert");
	fragment_program_ = program_cache.GetFromFile("deferred.frag", GL_FRAGMENT_SHADER, "assets/shaders/deferred.frag");
...
void MeshRenderer::Draw(float delta_time)
{
	...
	for (int i = 0; i < meshes_.size(); ++i)
	{
		...
		vertex_program_.SetUniform("u_proj", (void*)glm::value_ptr(camera.GetProj()));
		vertex_program_.SetUniform("u_view", (void*)glm::value_ptr(camera.GetView()));
		vertex_program_.SetUniform("u_vp", (void*)glm::value_ptr(camera.GetViewProj()));
